...
struct SwiftMachineWizardView: View {
    @Environment(\.swiftMachineWizardStoreFactory) private var swiftMachineWizardStoreFactory

    var body: some View {
        WithViewStore(store: swiftMachineWizardStoreFactory.make()) { store in
            content(for: store)
        }
    }

    private func content(for store: SwiftMachineWizardStore) -> some View {
        ZStack {
            LinearGradient(
                colors: [
    ...
    @ViewBuilder
    private func phaseContent(for store: SwiftMachineWizardStore) -> some View {
        if let machineName = store.state.machineName {
            wizardLayout(
                title: "Create the Initial State",
                subtitle: nil
            ) {
                InitialStateSetupStepView(store: store, machineName: machineName)
                    .id(machineName)
            }
        } else {
            wizardLayout(
                ...
                MachineDraftStepView(store: store)
            }
        }
    }
...
private struct MachineDraftStepView: View {
    ...
    private func submit() {
        ...
        store.send(.createEmptyStateMachine(name: trimmedMachineName))
    }
}
